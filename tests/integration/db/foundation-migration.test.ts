...
describe("migrate", () => {
  ...
  test("keeps the complete schema verified through email registration policy version 18", () => {
    const db = memoryDatabase();
    try {
      migrate(db);
      migrate(db);
      ...
      expect(
        ...
        { version: 17, applied_at: expect.any(Number) },
        { version: 18, applied_at: expect.any(Number) },
      ]);
      expect(
