...
export function createGitHubAppJwt(
  input: Readonly<{ appId: string; privateKey: Uint8Array; now: number }>,
): Result<string> {
  ...
  try {
    ...
    const payload = encode({
      iat: Math.max(0, Math.floor(input.now / 1000) - 60),
      exp: Math.floor(input.now / 1000) + 540,
      iss: input.appId,
    });
...
export async function requestInstallationToken(
  ...
  if (
    !/^[0-9]{1,32}$/.test(input.installationId) ||
    input.repositoryIds.length > 500 ||
    input.repositoryIds.some((id) => !/^[0-9]{1,32}$/.test(id))
  ) {
    return failure("GITHUB_APP_AUTH_INPUT_INVALID");
  }
  try {
    ...
    if (!response.ok)
      return failure(
        response.status === 403 ? "GITHUB_APP_PERMISSION_DENIED" : "GITHUB_APP_TOKEN_UNAVAILABLE",
        "SAME_INPUT",
      );
    ...
    if (
      ...
      return failure("GITHUB_APP_RESPONSE_INVALID");
    return {
      ok: true,
      value: {
        token,
...
export class GitHubInstallationTokenCache {
  ...
  async get(
    ...
    if (this.cached?.key === key && this.cached.value.expiresAt > input.now + 60_000)
