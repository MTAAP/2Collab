...
import { z } from "zod";
import { IdentifierSchema, InstantSchema } from "../../../shared/contracts/ids.ts";

export const MAXIMUM_RUNNER_FRAME_BYTES = 65_536;
export const MAXIMUM_HELLO_RANGES = 8;
...
export const ClientHelloSchema = z
  .object({
    ...
    ranges: z.array(ProtocolRangeSchema).min(1).max(MAXIMUM_HELLO_RANGES),
  })
  .strict()
...
export const RunnerMessageBodySchema = z.discriminatedUnion("kind", [
  ...
  z
    .object({
      ...
      operationDigest: z.string().regex(/^[0-9a-f]{64}$/),
    })
    .strict(),
  z
    .object({
