ALTER TABLE local_processes RENAME TO local_processes_v1;
...
CREATE TABLE local_processes (
  ...
  state TEXT NOT NULL CHECK (state IN ('RESERVED', 'STARTED', 'FAILED_TO_START', 'EXITED', 'UNKNOWN')),
  ...
  CHECK (
    (state = 'RESERVED' AND host IS NULL AND opaque_process_id IS NULL AND interaction IS NULL
      AND assurance IS NULL AND last_disposition IS NULL)
...
SELECT
  attempt_id, reservation_id, assignment_digest, state, host, opaque_process_id,
  interaction, assurance, last_disposition, created_at, updated_at
FROM local_processes_v1;

DROP TABLE local_processes_v1;
